...
class VisualAgent {
  ...
  async verifyStepExecution(
    ...
    switch (step.action) {
      ...
      case 'navigate':
        // For navigate actions, we consider success if the action completed
        // This handles both in-page navigation and external URL opening via local agent
        expectedChanges.push(`URL should open: ${step.value}`);
        // Always mark as success since navigate opens in new window/external app
        // The webapp's URL won't change, but the action is still successful
        actualChanges.push(`Opened/navigated to ${step.value}`);
        break;
      ...
      case 'verify':
        ...
        case 'custom':
          // Custom actions are assumed successful if the agent returns success
          // This includes opening applications, running scripts, etc.
          expectedChanges.push(`Custom action: ${step.description}`);
          actualChanges.push(`Custom action completed: ${step.description}`);
          break;
        ...
        default:
          if (domComparison.significantChange) {
            ...
            // For unknown actions, assume success if no errors
            actualChanges.push('Action executed');
          }
    }
  ...
  private async executeNavigate(step: PlanStep): Promise<{ success: boolean; result?: string; error?: string }> {
    ...
    // Use postMessage to open external URLs without killing the script context
    // This sends the URL to the parent window (if in iframe) or opens in new window
    const url = step.value;
    
    try {
      // Try to open via parent postMessage first (for iframe contexts)
      if (window.parent && window.parent !== window) {
        window.parent.postMessage({ type: 'OPEN_EXTERNAL_URL', data: { url } }, '*');
      } else {
        // Fallback: open in new window/tab to preserve script execution
        window.open(url, '_blank', 'noopener,noreferrer');
      }
      return { success: true, result: `Opened URL: ${step.value}` };
    } catch (error) {
