...
class VerifiedExecutor {
  ...
  private async executeStepWithVerification(
    ...
    const [beforeDOMState, beforeScreenshot] = await Promise.all([
      Promise.resolve(domStateCapture.captureSnapshot()),
    ...
    const [afterDOMState, afterScreenshot] = await Promise.all([
      Promise.resolve(domStateCapture.captureSnapshot()),
      this.captureScreenshot()
    ]);
