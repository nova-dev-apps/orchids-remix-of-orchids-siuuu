...
4. Verifying if the expected state matches what's shown`;
  ...
  /**
   * Convert screenshot to a text/code description for AI to understand without vision
   * This is used in Auto Mode so the AI can "see" screenshots as text
   */
  async convertScreenshotToCode(
    ...
    try {
      const cleanedBase64 = latest.base64.replace(/^data:image\/\w+;base64,/, '');
      
      const systemPrompt = `You are a screen reader AI. Your job is to convert screenshots into detailed text descriptions that another AI can understand WITHOUT seeing the image.

OUTPUT FORMAT - Provide a structured description like this:

\`\`\`screen-state
WINDOW: [Application name and window title]
SCREEN_SIZE: [Approximate dimensions if detectable]
...
UI_ELEMENTS:
1. [Element type] "[Label/Text]" at position (x, y) - [clickable/editable/static]
2. [Element type] "[Label/Text]" at position (x, y) - [clickable/editable/static]
...

VISIBLE_TEXT:
- "[Any important text visible on screen]"

CURRENT_STATE:
- Active element: [What's focused/selected]
- Dialogs/Popups: [Any open dialogs]
- Loading indicators: [Any spinners/progress bars]
- Error messages: [Any visible errors]

ACTIONABLE_TARGETS:
- Button "[name]" at (~x, ~y) - [what it does]
- Input "[name]" at (~x, ~y) - [current value if any]
- Link "[name]" at (~x, ~y) - [destination]
\`\`\`

Be precise about positions (estimate x,y coordinates). Be thorough - the receiving AI cannot see anything you don't describe.
${customPrompt ? `\nAdditional context: ${customPrompt}` : ''}`;
      ...
      if (isAnthropic) {
        ...
        requestBody = {
          ...
          messages: [{
            ...
            content: [
              ...
              {
                type: 'text',
                text: 'Describe this screenshot in detail so another AI can understand the screen state without seeing it.'
              }
            ]
          }]
        };
        ...
        requestBody = {
          contents: [{
            parts: [
              { text: `${systemPrompt}\n\nDescribe this screenshot in detail so another AI can understand the screen state without seeing it.` },
              {
                inline_data: {
                  mime_type: 'image/png',
                  data: cleanedBase64
      ...
      if (!response.ok) {
        console.error('[ScreenshotService] Screenshot to code conversion failed:', response.status);
        return null;
      }
      ...
      console.error('[ScreenshotService] Screenshot to code error:', error);
      return null;
    }
  }

  /**
   * Get cached screenshot code description or generate new one
   */
  private screenshotCodeCache: { description: string; screenshotId: string; timestamp: number } | null = null;
  
  async getScreenshotAsCode(
    config: AIVisionAnalysisConfig
  ): Promise<string | null> {
    const latest = this.getLatestScreenshot();
    if (!latest) return null;

    // Return cached if same screenshot
    if (this.screenshotCodeCache && 
        this.screenshotCodeCache.screenshotId === latest.id &&
        Date.now() - this.screenshotCodeCache.timestamp < 30000) {
      return this.screenshotCodeCache.description;
    }

    const description = await this.convertScreenshotToCode(config);
    if (description) {
      this.screenshotCodeCache = {
        description,
