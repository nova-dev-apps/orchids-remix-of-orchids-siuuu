...
const ChatHistory = ({ isOpen = false, onOpenChange, onNewChat, onLoadChat, userId: propUserId }: ChatHistoryProps) => {
  ...
  useEffect(() => {
    const loadSessions = async () => {
      ...
      if (!uid) {
        console.log('[ChatHistory] No userId available');
        return;
      }
      ...
      try {
        ...
        console.log('[ChatHistory] Loaded sessions:', storedSessions.length, storedSessions);
        setSessions(
    ...
    // Load sessions when panel opens or userId changes
    if (isOpen && (propUserId || userId)) {
      loadSessions();
    }
    
    // ALWAYS listen for session updates, reload when event fires regardless of open state
    const handleSessionUpdate = () => {
      console.log('[ChatHistory] Session saved event received, reloading...');
      loadSessions();
    };
